...
const STORAGE_KEY = "fpl-friends-tracker-v3";
...
function scoreParticipant(participant: Participant, table: Array<TeamRow & { gd: number; position: number }>): ScoredParticipant {
  ...
  if (participant.picks.managerSacked === "West Ham") score += 3;
...
function toEditableRows(standings: ApiStandingRow[]): TeamRow[] {
  return standings.map((row) => ({
    ...
    ga: row.ga,
    managerSacked: false,
    mostCardsLeader: row.team === "Chelsea",
  }));
}
    ...
    <main className="min-h-screen bg-slate-100 text-slate-900">
      <div className="mx-auto max-w-7xl px-4 py-8 md:px-8">
          ...
          <>
            <div className="mb-6 grid gap-4 md:grid-cols-4">
              ...
              <div className="rounded-3xl bg-white p-5 shadow-sm">
                ...
                <div className="mt-2 text-xl font-semibold">Chelsea</div>
                <div className="mt-3 space-y-1 text-sm text-slate-700">
                  {CARD_LEADERS.map((item) => (
                    <div key={item}>{item}</div>
                  ))}
                </div>
                <div className="mt-3 text-xs text-slate-500">Updated April 10, 2026 · Numbers may not be exact and are approximate values from Claude.</div>
              </div>
              <div className="rounded-3xl bg-white p-5 shadow-sm">
                <div className="text-sm text-slate-500">Managers Sacked</div>
                <div className="mt-3 space-y-1 text-sm text-slate-700">
                  {MANAGERS_SACKED.map((item) => (
                    <div key={item}>{item}</div>
                  ))}
                </div>
                <div className="mt-3 text-xs text-slate-500">Updated April 10, 2026</div>
              </div>
              <div className="rounded-3xl bg-white p-5 shadow-sm">
        ...
        <section className="mt-6 rounded-3xl bg-white p-5 shadow-sm">
          <h2 className="mb-4 text-xl font-semibold">Participant Entry</h2>
          <p className="mb-4 text-sm text-slate-600">Temporary form for adding players while we build the side-by-side picks table.</p>
